...
describe('tree', () => {
  ...
  it.each(middleVisitNodeTestData)(
    'middleVisitNode %j',
    ({ input, expected }) => {
      const root = preBuildNode(Queue.from(input));
  ...
  it.each(visitTreeByLevelTestData)(
    'visitTreeByLevel %j',
    ({ input, expected }) => {
      const root = preBuildNode(Queue.from(input));
  ...
  test('findNextNode', () => {
    const [root, node2, node3, node4, node6, node7, node8] = [
      1, 2, 3, 4, 6, 7, 8,
    ].map((val) => new TreeNodeWithParent(val));
    node7.parent = node6;
    node8.parent = node6;
    node6.parent = node3;
    node4.parent = node2;
  ...
  describe('morris', () => {
    ...
    test('morris 1', () => {
      expect(morris(head).map(({ val }) => val)).toEqual([
        1, 2, 4, 2, 5, 1, 3, 6, 3, 7,
      ]);
    });
  ...
  describe('serializeByLevel,deserializeByLevel', () => {
    ...
    test('[1, 2, null, 3, 6, 4, null, 7, 8, null, null, 9]', () => {
      ...
      expect(serializeByLevel(head)).toEqual([
        1,
        2,
        null,
        3,
