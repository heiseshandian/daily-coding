...
function addEffectsToRun(
  effectsToRun: Set<ActiveEffect>,
  effects: Set<ActiveEffect> | undefined | null
) {
  if (!effects) {
    return;
  }
      ...
      if (receiver[ActiveTargetFlags.Raw] === target) {
        ...
        if (
          ...
          trigger(
            target,
            key,
            hasOwnProp ? TriggerTypes.Set : TriggerTypes.Add,
            newValue
          );
        }
      }
...
export function effect<T extends (...args: any[]) => any>(
  fn: T,
  options: ActiveEffectOptions = {}
) {
...
export function proxyRefs<T extends object>(
  ...
  return new Proxy<any>(target, {
    get(target, key, receiver) {
      ...
      return (value as Ref)[ActiveTargetFlags.Raf]
        ? (value as Ref).value
        : value;
    },
    set(target, key, newValue, receiver) {
