...
export function getHappyString(n: number, k: number): string {
    ...
    const backtrack = (i: number, s: string) => {
        if (i === n) {
            happyStrings.push(s);
            return;
        }
...
export function findDifferentBinaryString(nums: string[]): string {
    ...
    const backtrack = (len: number, sequence: string) => {
        if (ret.length) {
            return;
        }
        if (len === nums.length) {
        ...
        backtrack(len + 1, sequence + '1');
        backtrack(len + 1, sequence + '0');
    };
