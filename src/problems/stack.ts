...
function countRepeatTimes(stack: number[], repeatNum: number, depth: number) {
  ...
  const n = stack.pop()!;
  const count =
    countRepeatTimes(stack, repeatNum, depth - 1) + (n === repeatNum ? 1 : 0);
  stack.push(n);
  return count;
}
...
/*
https://leetcode.com/problems/simplify-path/description/?envType=problem-list-v2&envId=o5cftq05
71. Simplify Path
...
export function simplifyPath(path: string): string {
  ...
  while (i >= 0) {
    if (segments[i] === '' || segments[i] === '.') {
      ...
      stack.push('..');
    } else if (stack.length && stack.at(-1) === '..') {
      stack.pop();
    } else {
