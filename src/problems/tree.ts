...
function isEqualTree(node1: TreeNode | null, node2: TreeNode | null): boolean {
  ...
  return (
    isEqualTree(node1.left, node2.right) && isEqualTree(node1.right, node2.left)
  );
}
...
export function pathSum(root: TreeNode | null, targetSum: number): number[][] {
  ...
  const backtracking = (node: TreeNode | null, path: number[], sum: number) => {
    if (!node) {
      return;
    }
    if (isLeaf(node)) {
...
*/
export function minimumOperations(root: TreeNode): number {
  const queue: TreeNode[] = Array(1e5 + 1);
  let left = 0;
  let right = 0;
  queue[right++] = root;
  ...
  while (left < right) {
    ...
    if (node === curEnd) {
      const sorted = level.slice().sort((a, b) => a - b);
      const map = {};
      ...
      for (let i = 0; i < level.length; i++) {
        if (level[i] !== sorted[i]) {
          minOp++;
          const idx = map[sorted[i]];
          [level[i], level[idx]] = [level[idx], level[i]];
          [map[level[i]], map[level[idx]]] = [map[level[idx]], map[level[i]]];
        }
      }
...
export function recoverFromPreorder(traversal: string): TreeNode | null {
  const NUM_REG = /(\d+)/;
  const getRegByLevel = cache((level: number) =>
    level === 0 ? NUM_REG : new RegExp(`^(?:-{${level}})(\\d+)`)
  );

  const build = (level: number): TreeNode | null => {
    if (!traversal) {
      return null;
    }

    const reg = getRegByLevel(level);
    const match = traversal.match(reg);
    if (!match) {
      return null;
    }

    const [m, val] = match;
    traversal = traversal.slice(m.length);
    const head = new TreeNode(+val);
    head.left = build(level + 1);
    head.right = build(level + 1);
...
export function constructFromPrePost(
  ...
  const build = (
    ...
    const head = new TreeNode(preorder[preStart]);
    const leftLen = postMap[preorder[preStart + 1]] - postStart + 1;
    head.left = build(
      preStart + 1,
      preStart + leftLen,
      postStart,
