...
function bagProcess(
  ...
  if (restWeight - weights[i] >= 0) {
    p1 =
      values[i] + bagProcess(weights, values, i + 1, restWeight - weights[i]);
  }
...
function walkProcess(n: number, p: number, restK: number, i: number): number {
  ...
  // 向左走或者向右走
  return (
    walkProcess(n, p, restK - 1, i + 1) + walkProcess(n, p, restK - 1, i - 1)
  );
}
...
export function getPlusOrMinusCountDp(arr: number[], target: number): number {
  ...
  for (let i = arr.length - 1; i >= 0; i--) {
    for (let rest = 0; rest <= sum; rest++) {
      // 两种选择
      const p1 =
        Math.abs(rest - arr[i]) <= sum ? prevDp[Math.abs(rest - arr[i])] : 0;
...
export function minExtraChar(s: string, dictionary: string[]): number {
  ...
  for (let i = 1; i <= n; i++) {
    ...
    for (let j = i - 1; j >= 0; j--) {
      dp[i] = Math.min(dp[i], dp[j] + (set.has(s.slice(j, i)) ? 0 : i - j + 1));
    }
  }
...
export function canIWin(
  ...
  const dfs = (state: number, rest: number): boolean => {
    ...
    for (let i = 0; i < maxChoosableInteger; i++) {
      ...
      if (i + 1 >= rest || dfs(state | (1 << i), rest - i - 1) === false) {
        visited[state] = true;
        return true;
      }
    }
...
export function mostPoints(questions: number[][]): number {
  ...
  for (let i = n - 2; i >= 0; i--) {
    ...
    dp[i] = Math.max(point + (dp[i + brainpower + 1] || 0), dp[i + 1] || 0);
  }
