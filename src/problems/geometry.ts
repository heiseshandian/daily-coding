...
*/
export function checkValidCuts(_n: number, rectangles: number[][]): boolean {
  const x = Array(rectangles.length);
  const y = Array(rectangles.length);
  for (let i = 0; i < rectangles.length; i++) {
    const [startx, starty, endx, endy] = rectangles[i];
    x[i] = [startx, endx];
    y[i] = [starty, endy];
  }

  return mergeIntervals(x).length >= 3 || mergeIntervals(y).length >= 3;
}
