...
describe('dp', () => {
  ...
  it.each(countWalkMethodsTestData)(
    'countWalkMethods %j',
    ({ input: { n, m, k, p }, expected }) => {
  ...
  it.each(maxCommonSubsequenceTestData)(
    'maxCommonSubsequence',
  ...
  it.each(getCoffeeTimeTestData)(
    'getCoffeeTime',
    ({ input: { arr, a, b }, expected }) => {
  ...
  it.each(getHorseMethodsTestData)(
    'getHorseMethods',
    ({ input: { a, b, k }, expected }) => {
  ...
  it.each(getMinValueOfColorTestData)(
    'getMinValueOfColor',
    ({ input, expected }) => {
  ...
  it.each(isInterleaveTestData)(
    'isInterleave',
    ({ input: { str1, str2, str3 }, expected }) => {
      expect(isInterleave(str1, str2, str3)).toBe(expected);
    }
  );

  it.each(editDistanceTestData)(
    'getMinEditDistance',
    ({ input: { word1, word2 }, expected }) => {
  ...
  it.each(minimumTotalTestData)('minimumTotal', ({ input, expected }) => {
    expect(minimumTotal(input)).toBe(expected);
    expect(minimumTotal2(input)).toBe(expected);
    expect(minimumTotal3(input)).toBe(expected);
  });
});
