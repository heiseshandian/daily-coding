export function convertStr2Int(s: string): number {
  ...
  for (; i < s.length; i++) {
    ...
    if (
      result > MAX_VALUE_DIVIDED_BY_10 ||
      (result == MAX_VALUE_DIVIDED_BY_10 && cur > MAX_VALUE % 10)
    )
      return MAX_VALUE;
    if (
      result < MIN_VALUE_DIVIDED_BY_10 ||
      (result == MIN_VALUE_DIVIDED_BY_10 && cur < MIN_VALUE % 10)
    )
      return MIN_VALUE;
